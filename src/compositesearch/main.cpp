...
int main(int argc, char * argv[])
{
	...
	runLoadNetwork(fileIn, geneList, pidentLimit, evalueLimit, refPositionsList, refEdges, refGenes, refND, refGIV, nCpu);
	end = time(NULL);
	if( verbose == 1 )
	{
		cout << "END   - " << asctime(localtime(&end)) << endl;
	...
	if( found == "composites" )
	{
		...
		// Detect composites
		runComposites(refGenes, refEdges, refGeneFamilies, refND, MAX_OVERLAP, minCov, compositeFamMinSize, componentFamMinSize, nCpu);
		// Compute composite Families
		runCompositeFamilies(refGenes, refGeneFamilies, nCpu);
		end = time(NULL);
		// Merge all output files in one
		mergeFiles(nCpu, "composites", fileOut);
		if( verbose == 1 )
		{
			cout << "END   - " << asctime(localtime(&end)) << endl;;
		}
	...
	else if ( found == "fastcomposites" )
	{
		...
		runFastComposites( refGenes, refEdges, refND, MAX_OVERLAP, nCpu);
		end = time(NULL);
		// Merge all output files in one
		mergeFiles( nCpu, "fastcomposites", fileOut);
		if( verbose == 1 )
		{
